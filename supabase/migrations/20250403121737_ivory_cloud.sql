/*
  ...
  1. Changes
    - Simplify overlap checking logic
    - Remove interval addition from end time check
    - Keep time slot generation and other checks the same
    
  2. Notes
    - Should fix issue where slots after a booking were incorrectly marked as unavailable
    - Maintains proper time slot availability checking
*/
      ...
      WHEN gs.slot_time::time >= v_start_hour 
        ...
        AND NOT EXISTS (
          ...
          WHERE (
            ...
            (gs.slot_time::text BETWEEN b.start_time AND b.end_time)
          )
        )
      THEN true
      ELSE false
