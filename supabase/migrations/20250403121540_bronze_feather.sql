/*
  # Fix Booking Overlap Check

  1. Changes
    - Simplify overlap checking logic to properly handle gaps between bookings
    - Fix issue where slots after a booking were incorrectly marked as unavailable
    - Maintain proper time slot generation
    
  2. Notes
    - Slots between bookings (like 12 PM after a 9-11 AM booking) will now show as available
    - Maintains all other availability rules
*/
      ...
      WHEN gs.slot_time::time >= v_start_hour 
        ...
        AND NOT EXISTS (
          ...
          WHERE (
            ...
            (gs.slot_time BETWEEN b.start_time AND b.end_time)
            OR
            -- Check if slot ends during a booking
            ((gs.slot_time::time + interval '1 hour')::text BETWEEN b.start_time AND b.end_time)
          )
        )
      THEN true
      ELSE false
