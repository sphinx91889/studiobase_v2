/*
  # Fix ambiguous time_slot reference

  1. Changes
    - Drop existing get_available_time_slots function
    - Create improved version with unambiguous column references
    - Add better time slot validation
...
BEGIN
  ...
  RETURN QUERY
  WITH RECURSIVE generated_slots AS (
    SELECT v_start_hour::text as slot_time
    UNION ALL
    SELECT to_char((slot_time::time + interval '1 hour')::time, 'HH24:MI')
    FROM generated_slots
    WHERE slot_time::time < v_end_hour - interval '1 hour'
  )
  SELECT 
    ...
    CASE
      ...
      WHEN gs.slot_time::time >= v_start_hour 
        ...
        AND NOT EXISTS (
          ...
          AND (
            (sb.start_time::time <= gs.slot_time::time 
              AND sb.end_time::time > gs.slot_time::time)
            OR 
            (sb.start_time::time < (gs.slot_time::time + interval '1 hour')
              AND sb.end_time::time >= (gs.slot_time::time + interval '1 hour'))
            OR
            (sb.start_time::time >= gs.slot_time::time 
              AND sb.end_time::time <= (gs.slot_time::time + interval '1 hour'))
          )
