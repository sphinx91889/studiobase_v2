/*
  # Fix studio owner email handling

  1. Changes
    - Update get_studio_owner_email function to use studio's email field
    - Remove hardcoded email address
    - Add better error handling and logging
    
  2. Security
    - Maintains existing security model
    - Uses proper relationship chain
*/
...
BEGIN
  ...
  SELECT 
    rc.studio_email INTO v_owner_email
  ...
  IF v_owner_email IS NULL THEN
    RAISE WARNING 'No email found for room_id: %. Check studio email field.', p_room_id;
  ELSE
    RAISE NOTICE 'Found studio email: %', v_owner_email;
  END IF;
