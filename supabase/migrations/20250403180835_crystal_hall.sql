/*
  # Fix Booking Time Slot Availability

  1. Changes
    - Improve time slot availability checking
    - Fix booking overlap detection
    - Handle multi-hour bookings correctly
    - Ensure proper time range comparisons
    
  2. Notes
    - Uses proper timestamp comparisons
    - Accounts for booking duration
    - Maintains existing room availability rules
*/
      ...
      WHEN
        ...
        AND NOT EXISTS (
          ...
          WHERE 
            ...
            OR
            -- Check if any part of the booking overlaps with this slot
            (
              b.start_time <= s.slot_time::time 
              AND b.end_time > s.slot_time::time
            )
            OR
            (
              b.start_time >= s.slot_time::time 
              AND b.start_time < (s.slot_time::time + interval '1 hour')
            )
        )
      THEN true
      ELSE false
